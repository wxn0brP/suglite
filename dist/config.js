...
import { getSuglitePath, loadJson, loadPredefinedConfig } from "./config.utils.js";
import { groupArguments } from "./groupArgs.js";
...
export const argv = await yargs(hideBin(rawArgs))
    ...
    .positional("name", { type: "string", description: "Predefined config reference name" }), (arg) => {
...
if (existsSync(argv.file)) {
    const localConfig = loadJson(argv.file);
    if (localConfig.ref) {
        let refConfig = {};
...
if (argv.cmd)
    config.cmd = argv.cmd;
if (argv.args)
    config.args = argv.args;
if (argv.watch)
    config.watch = argv.watch;
if (argv.ignore)
    config.ignore = argv.ignore;
if (argv.restart_cmd)
    config.restart_cmd = argv.restartCmd;
if (argv.history)
    config.history = argv.history;
if (argv.delay)
    config.delay = argv.delay;
if (argv.trustedShells)
    config.trustedShells = argv.trustedShells;
...
export let processedCmd = config.cmd +
