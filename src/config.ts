	...
	.command(
		...
		yargs =>
			yargs.positional("name", {
				type: "string",
				description: "Predefined config reference name",
			}),
		arg => {
			const file =
				arg.file && arg.file !== "default" ? arg.file : "suglite.json5";
			if (existsSync(file)) {
...
const globalConfig =
	existsSync(globalConfigPath) && !argv.igc ? loadJson(globalConfigPath) : {};
...
if (argv.trustedShells)
	mainConfig.trustedShells = argv.trustedShells as string[];
if (argv.server) mainConfig.server = argv.server;

// @ts-ignore
if (typeof mainConfig.watch === "string")
	mainConfig.watch = [
		mainConfig.watch,
	];
// @ts-ignore
if (typeof mainConfig.ignore === "string")
	mainConfig.ignore = [
		mainConfig.ignore,
	];
...
if (
	(processedCmd.startsWith(`"`) && processedCmd.endsWith(`"`)) ||
	(processedCmd.startsWith(`'`) && processedCmd.endsWith(`'`))
) {
